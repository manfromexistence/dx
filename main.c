...
// gcc -O3 -march=native -o file_creator file_creator.c -lpthread
...
// Pass all necessary data to the thread, including the directory file descriptor.
typedef struct {
    int start_index;
    int end_index;
    int dir_fd; // File descriptor for the FOLDER directory
    const char *content;
    size_t content_len;
} ThreadArgs;
...
// Worker using write() and openat()
void *create_files_worker(void *arg) {
    ThreadArgs *args = (ThreadArgs *)arg;
    char filename[256]; // Buffer for just the filename, not the full path.
    ...
    for (int i = args->start_index; i < args->end_index; i++) {
        ...
        // Use openat() with the directory descriptor. This is faster.
...
void *overwrite_files_mmap_worker(void *arg) {
    ...
    for (int i = args->start_index; i < args->end_index; i++) {
        ...
        // Use openat() for the performance gain.
        int fd = openat(args->dir_fd, filename, O_RDWR | O_CREAT, 0644);
        if (fd == -1) continue;
...
int main() {
    ...
    // Open the target directory just ONCE.
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        args[i].dir_fd = dir_fd; // Pass the directory descriptor to each thread.
    ...
    // Clean up the directory descriptor.
    close(dir_fd);
