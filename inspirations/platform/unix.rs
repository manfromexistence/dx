...
unsafe fn get_dimensions_any() -> winsize {
    ...
    let mut result = unsafe { ioctl(STDOUT_FILENO, TIOCGWINSZ, &mut window) };

    if result == -1 {
        window = unsafe { zeroed() };
        result = unsafe { ioctl(STDIN_FILENO, TIOCGWINSZ, &mut window) };
        if result == -1 {
            window = unsafe { zeroed() };
            result = unsafe { ioctl(STDERR_FILENO, TIOCGWINSZ, &mut window) };
            if result == -1 {
...
unsafe fn get_dimensions_out() -> winsize {
    let mut window: winsize = unsafe { zeroed() };
    let result = unsafe { ioctl(STDOUT_FILENO, TIOCGWINSZ, &mut window) };
...
unsafe fn get_dimensions_in() -> winsize {
    let mut window: winsize = unsafe { zeroed() };
    let result = unsafe { ioctl(STDIN_FILENO, TIOCGWINSZ, &mut window) };
...
unsafe fn get_dimensions_err() -> winsize {
    let mut window: winsize = unsafe { zeroed() };
    let result = unsafe { ioctl(STDERR_FILENO, TIOCGWINSZ, &mut window) };
